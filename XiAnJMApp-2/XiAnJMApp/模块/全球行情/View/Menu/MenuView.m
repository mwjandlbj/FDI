...
-(instancetype)initWithDependencyView:(UIView *)dependencyView MenuView:(UIView *)leftmenuView isShowCoverView:(BOOL)isCover{
    ...
    if(self = [super init]){
        ...
        if((CGRectEqualToRect(_coverViewframe, CGRectZero)) == NO){
            ...
            CGRect rect;
            rect.origin.x=leftmenuView.frame.origin.x;
            rect.origin.y=leftmenuView.frame.origin.y;
            rect.size=leftmenuView.frame.size;
            
            
            self.menuViewframe = rect;
        }else{
        }
...
//    self.coverView.frame = CGRectMake(0, HEIGHT_Nav, kSCREEN_WIDTH, self.menuViewframe.size.height);
    self.coverView.frame = CGRectMake(0, HEIGHT_Nav, kSCREEN_WIDTH, kSCREEN_HEIGHT);
    ...
    if(UIGestureRecognizerStateBegan == gesture.state ||
       UIGestureRecognizerStateChanged == gesture.state){
        ...
        if(translation.x <= self.menuViewframe.size.width){// && self.leftMenuView.frame.origin.x != 0
            ...
            self.coverView.frame    = CGRectMake(self.leftMenuView.frame.size.width+x, HEIGHT_Nav,kSCREEN_WIDTH-self.leftMenuView.frame.size.width-x, self.menuViewframe.size.height);
    ...
    if(UIGestureRecognizerStateBegan == recognizer.state ||
       UIGestureRecognizerStateChanged == recognizer.state){
        ...
        if(Place <= self.leftMenuView.frame.size.width &&  Place >0){
            ...
            self.coverView.frame    = CGRectMake(self.leftMenuView.frame.size.width-Place, HEIGHT_Nav,kSCREEN_WIDTH-self.leftMenuView.frame.size.width+Place, self.menuViewframe.size.height);
            ...
            self.coverView.frame    = CGRectMake(self.leftMenuView.frame.size.width, HEIGHT_Nav,kSCREEN_WIDTH-self.leftMenuView.frame.size.width, self.menuViewframe.size.height);
            self.coverView.alpha    = CoverViewAlpha;
        }
...
-(void)coverTap{
    ...
    [UIView animateKeyframesWithDuration:0.3 delay:0 options:UIViewKeyframeAnimationOptionLayoutSubviews animations:^{
        self.leftMenuView.frame = CGRectMake(-self.menuViewframe.size.width, HEIGHT_Nav, self.menuViewframe.size.width, self.menuViewframe.size.height);
        self.coverView.frame    = CGRectMake(0, HEIGHT_Nav,kSCREEN_WIDTH, kSCREEN_HEIGHT);
...
-(void)removeCoverAndMenuView{
    ...
    self.leftMenuView.frame = CGRectMake(-self.leftMenuView.frame.size.width, HEIGHT_Nav, self.leftMenuView.frame.size.width, kSCREEN_HEIGHT);
    self.coverView.frame    = CGRectMake(0, HEIGHT_Nav,kSCREEN_WIDTH, kSCREEN_HEIGHT);
    self.coverView.alpha    = 0.0;
...
-(CGRect)coverViewframe{
    ...
    if((CGRectEqualToRect(_coverViewframe, CGRectZero)) == YES){
        ...
        _coverViewframe = CGRectMake(self.menuViewframe.size.width, self.menuViewframe.origin.y+HEIGHT_Nav, kSCREEN_WIDTH - self.menuViewframe.size.width, self.menuViewframe.size.height);
    }
