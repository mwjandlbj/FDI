//
//  InquirePresetBidListrspTableViewCell.m
//  XiAnJMApp
//
...
@implementation InquirePresetBidListrspTableViewCell

+ (instancetype)cellWithTableView:(UITableView *)tableView WithNumberOfLabels:(NSInteger)number {
    
    static NSString *identifier = @"cell";
    // 1.缓存中取
    InquirePresetBidListrspTableViewCell *cell = [tableView dequeueReusableCellWithIdentifier:identifier];
    // 2.创建
    if (cell == nil) {
        cell = [[InquirePresetBidListrspTableViewCell alloc]initWithStyle:UITableViewCellStyleDefault reuseIdentifier:identifier];
...
- (void)setModel:(PresetBidListrspModel *)model{
    if (_model != model) {
        _model = model;
        [self upData];
    }
