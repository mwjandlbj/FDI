...
@implementation LimitBidListrspModel
...
+ (instancetype)dicInitWithDic:(NSDictionary *)dic {
    return [[LimitBidListrspModel alloc] initWithDic:dic];
}
...
- (void)getModelDatas{
    ...
    if ([self.LimitType intValue] == 0) {
        NSString  *str = @"止损";
        [datas addObject:str];
    }else if ([self.LimitType intValue] == 1){
        NSString  *str = @"止盈";
        [datas addObject:str];
    }else {
        NSString  *str = @"止损止盈";
    ...
    if ([self.LimitResult intValue] == 1) {
        NSString  *str = @"成功";
        [datas addObject:str];
    }else {
        NSString  *str = @"--";
