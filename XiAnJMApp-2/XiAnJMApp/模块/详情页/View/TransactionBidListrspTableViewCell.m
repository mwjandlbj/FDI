//
//  TransactionBidListrspTableViewCell.m
//  XiAnJMApp
//
...
@implementation TransactionBidListrspTableViewCell

+ (instancetype)cellWithTableView:(UITableView *)tableView WithNumberOfLabels:(NSInteger)number {
    
    static NSString *identifier = @"cell";
    // 1.缓存中取
    TransactionBidListrspTableViewCell *cell = [tableView dequeueReusableCellWithIdentifier:identifier];
    // 2.创建
    if (cell == nil) {
        cell = [[TransactionBidListrspTableViewCell alloc]initWithStyle:UITableViewCellStyleDefault reuseIdentifier:identifier];
...
- (void)setModel:(TransactionBidListrspModel *)model{
    if (_model != model) {
        _model = model;
        [self upData];
    }
