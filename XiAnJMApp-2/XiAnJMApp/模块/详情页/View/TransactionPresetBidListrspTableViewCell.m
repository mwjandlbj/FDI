//
//  TransactionPresetBidListrspTableViewCell.m
//  XiAnJMApp
//
...
@implementation TransactionPresetBidListrspTableViewCell

+ (instancetype)cellWithTableView:(UITableView *)tableView WithNumberOfLabels:(NSInteger)number {
    
    static NSString *identifier = @"cell";
    // 1.缓存中取
    TransactionPresetBidListrspTableViewCell *cell = [tableView dequeueReusableCellWithIdentifier:identifier];
    // 2.创建
    if (cell == nil) {
        cell = [[TransactionPresetBidListrspTableViewCell alloc]initWithStyle:UITableViewCellStyleDefault reuseIdentifier:identifier];
...
- (void)setModel:(TransactionPresetBidListrspModel *)model{
    if (_model != model) {
        _model = model;
        [self upData];
    }
